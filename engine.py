...
SELF_TUNING = {
    ...
    "free_agency_directive_interval": 60,   # Was 600 — directives must queue before mutation fires
}
...
class AutonomousInstitutionEngine:
    ...
    def _persist_volatile_state(self):
        """Persist in-memory toggles to survive Railway restarts."""
        ...
        try:
            ...
            with open("/tmp/farl_volatile.json", "w") as f:
                _json.dump(state, f)
            logger.info("VOLATILE_STATE_SAVED: free_agency=%s", self.free_agency_enabled)
        except Exception as e:
            logger.warning("VOLATILE_SAVE_FAILED: %s", e)

    def _load_volatile_state(self):
        """Restore volatile state from file on restart."""
        import json as _json
        try:
            ...
            logger.info("VOLATILE_STATE_LOADED: free_agency=%s mode=%s",
                        self.free_agency_enabled, self.autonomy_mode)
    ...
    def _derive_objective(self) -> str:
        # Free agency: agent directive queue (highest priority)
