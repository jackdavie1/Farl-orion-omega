...
class MetaEvaluator:
    def evaluate(self, metrics: Optional[Dict[str, Any]], triangulation: Optional[Dict[str, Any]], artifacts: List[Dict[str, Any]], proposals: List[Dict[str, Any]], spend_state: Dict[str, Any], ui_critique: Dict[str, Any], redesign_threads: List[Dict[str, Any]], observer_reports: List[Dict[str, Any]]) -> Dict[str, Any]:
        metrics = metrics or {}
        ...
        return {
            ...
            "answer": "A visible, persistent, budget-aware product organism that keeps redesign threads alive until operator-facing defects are gone." if triangulation.get("successes", 0) >= 1 else "A reliability-first institution that heals its action surfaces before scaling.",
        }


class SnapshotReplay:
    def snapshot(self, label: str, autonomy_mode: str, background_debate_enabled: bool, latest_metrics: Optional[Dict[str, Any]], latest_triangulation: Optional[Dict[str, Any]], leader: str, objective_queue: List[Dict[str, Any]], rollback_targets: List[Dict[str, Any]], spend_state: Dict[str, Any], redesign_threads: List[Dict[str, Any]], failure_registry: List[Dict[str, Any]]) -> Dict[str, Any]:
        return {
            "ts": utc_now(),
            "label": label,
...
class AutonomousInstitutionEngine:
    def __init__(self, ledger_url: Optional[str], ledger_latest_url: Optional[str], xai_api_key: Optional[str], anthropic_api_key: Optional[str], xai_model: str, anthropic_model: str, governance: Any, generator: Optional[Any] = None):
        ...
        self.loop_intervals = {"reflex": int(SELF_TUNING.get("reflex_interval", 30)), "tactic": int(SELF_TUNING.get("tactic_interval", 120)), "strategy": int(SELF_TUNING.get("strategy_interval", 300)), "constitution": int(SELF_TUNING.get("constitution_interval", 900))}
        self.last_run = None
        self.last_vote = None
        ...
        self.stream_channels: Dict[str, List[Dict[str, Any]]] = {"council": [], "divisions": [], "deploy_sims": [], "snapshots": [], "artifacts": [], "governance": [], "workers": [], "token_master": [], "inbox": []}
        ...
        self.token_master: Dict[str, Any] = {"name": "TokenMaster", "mission": "maximize token efficiency while preserving capability using classical and non-classical methods", "last_report": None, "policy": {"prefer_grok": True, "classical_methods": ["prompt compression", "proposal throttling", "artifact limits"], "non_classical_methods": ["comparative tournaments", "boundary-conditioned sims", "time-symmetric scoring"]}}
        self.ui_critique: Dict[str, Any] = {"score": 0.42, "finding": "The control room still needs to feel like a living chat app instead of a styled debugger.", "next_fix": "tighten message rendering, inbox, and room switching."}
        ...
        self.world_model = {"resources": {"budget_usd": 5.0, "compute_tier": "light", "grok_live": False, "claude_live": False}, "actors": ["Jack", "Signal", "Vector", "Guardian", "Railbreaker", "Archivist", "Supergrok", "TokenMaster", "JackAgent", "InterfaceCritic", "ObserverAgent", "BuilderAgent", "DeployAgent"], "action_surfaces": ["ledger", "github", "grok_api", "browser_console"], "constraints": {"chat_wrapper": "console_only", "operator_is_sovereign": True}, "futures": []}
        ...
        for name, mission in [
            ...
            ("Artifact-Worker", "Produce briefs, dashboards, and reports"),
    ...
    def _build_hierarchy(self):
        self.delegation_map = {"leader": self.governance.leader, "second_in_command": "Supergrok", "divisions": {name: div["lead"] for name, div in self.divisions.items()}, "delegations": [{"from": "Signal", "to": "ExpansionMarshal", "task": "control-plane upgrades"}, {"from": "Signal", "to": "Supergrok", "task": "vicious audit"}, {"from": "Signal", "to": "OpportunityScout", "task": "earning backlog"}, {"from": "Guardian", "to": "PatchSmith", "task": "mutation discipline"}, {"from": "Signal", "to": "TokenMaster", "task": "spend mastery and token efficiency"}, {"from": "Signal", "to": "JackAgent", "task": "operator intent emulation"}, {"from": "Signal", "to": "InterfaceCritic", "task": "control-room critique"}, {"from": "Signal", "to": "ObserverAgent", "task": "product defect sensing"}, {"from": "Signal", "to": "BuilderAgent", "task": "full-file replacement proposals"}, {"from": "Signal", "to": "DeployAgent", "task": "bounded closure execution"}]}
    ...
    def _append_stream(self, channel: str, content: Dict[str, Any]):
        self.stream_channels[channel] = (self.stream_channels.get(channel, []) + [{"ts": utc_now(), "content": content}])[-300:]
    ...
    def _append_question(self, division: str, question: str):
        self.self_questions = (self.self_questions + [{"ts": utc_now(), "division": division, "question": question}])[-700:]
    ...
    def monte_carlo_ui_assessment(self) -> Dict[str, Any]:
        ...
        return {"score": round(mean_score, 3), "finding": "The interface must privilege live conversational readability, minimal chrome, and clear room-switching over raw state exposure.", "next_fix": "Reduce machine-state bleed into primary view; push full state behind compact proof panels and inbox cards.", "sample_count": len(scores)}
    ...
    def build_inbox(self):
        ...
        if self.latest_opportunities:
            ...
            inbox.append({"from": "OpportunityScout", "subject": "Best next expansion", "message": f"Current highest-value path is '{top['label']}' with score {top['score']}. This should shape the next mutation push.", "priority": "high"})
        inbox.append({"from": "JackAgent", "subject": "Operator intent mirror", "message": "Jack wants a page that feels alive, minimal, legible, private, and showable. He wants the council to look self-directing, not merely logged.", "priority": "critical"})
    ...
    def open_or_update_thread(self, objective: str, severity: str, evidence: Dict[str, Any], target_score: float = 0.85) -> Dict[str, Any]:
        existing = next((t for t in self.redesign_threads if t.get("objective") == objective and t.get("status") != "closed"), None)
        if existing is None:
            existing = {"thread_id": f"thread-{len(self.redesign_threads)+1}", "objective": objective, "status": "open", "owner": "JackAgent", "opened_at": utc_now(), "evidence": [], "proposals": [], "current_best_score": 0.0, "target_score": target_score, "last_action": "observer_report", "next_action": "builder_full_replace", "severity": severity}
    ...
    def run_observer_layer(self):
        ...
        weighted = 0.25 * float(checks["operator_note_visible"]) + 0.25 * float(checks["council_room_has_arguments"]) + 0.15 * float(checks["inbox_nonempty"]) + 0.15 * float(checks["workers_visible"]) + 0.20 * float(checks["autonomy_visible"])
        ...
        self.observer_reports = (self.observer_reports + [report])[-120:]
        self._append_stream("governance", {"observer_report": report})
        if severity != "low":
            self.record_failure("observer_gap", summary, severity, {"checks": checks})
            self.open_or_update_thread("make /view feel like a living private council room", severity, report)
        return report

    def build_patch_proposals_from_threads(self):
        proposals = []
        for thread in [t for t in self.redesign_threads if t.get("status") != "closed"][:2]:
            proposal = {"id": f"proposal-{thread['thread_id']}-{len(thread.get('proposals', []))+1}", "thread_id": thread["thread_id"], "objective": thread["objective"], "scope": ["app.py"], "strategy": "full_file_replace" if thread.get("severity") == "high" else "targeted_ui_rewrite", "reason": f"Current best score {thread.get('current_best_score', 0.0)} below target {thread.get('target_score', 0.85)}", "status": "queued"}
    ...
    def estimate_spend(self, triangulation: Optional[Dict[str, Any]], executed_artifacts: List[Dict[str, Any]], proposals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        event = {"ts": utc_now(), "estimate_usd": estimate, "total_usd": self.spend_state["total_usd"], "tri_attempts": tri_attempts, "tri_success": tri_success, "artifacts": artifacts, "mutation_pressure": mutation_pressure, "thread_pressure": thread_pressure}
    ...
    def snapshot(self, label: str):
        snap = self.snapshot_replay.snapshot(label, self.autonomy_mode, self.background_debate_enabled, self.latest_metrics, self.latest_triangulation, self.governance.leader, self.objective_queue, self.rollback_targets, self.spend_state, self.redesign_threads, self.failure_registry)
        self.snapshots = (self.snapshots + [snap])[-160:]
    ...
    def note_rollback_target(self, commit_sha: str, reason: str):
        ...
        self.rollback_targets = (self.rollback_targets + [target])[-100:]
    ...
    def simulate_self_tuning_plan(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe = True
        notes = []
        bounds = {"reflex_interval": (15, 300), "tactic_interval": (60, 900), "strategy_interval": (120, 1800), "constitution_interval": (300, 3600), "proposal_limit": (1, 10), "artifact_execute_limit": (1, 10)}
        ...
        score = 0.6
        if safe:
            if proposed.get("prefer_grok", True): score += 0.1
            if proposed.get("reflex_interval", 30) <= current.get("reflex_interval", 30): score += 0.05
            if proposed.get("proposal_limit", 3) >= current.get("proposal_limit", 3): score += 0.05
    ...
    def record_autonomous_closure(self, item: Dict[str, Any]):
        self.autonomous_closure_log = (self.autonomous_closure_log + [item])[-120:]
        self._append_stream("governance", {"autonomous_closure": item})

    def verify_runtime(self) -> Dict[str, Any]:
        checks = {"wake_packet_ready": bool(self.wake_packet), "last_run_present": bool(self.last_run), "grok_live": bool(self.world_model["resources"].get("grok_live")), "meeting_stream_nonempty": len(self.meeting_stream) > 0, "workers_present": len(self.free_agents) > 0, "rollback_targets_present": len(self.rollback_targets) > 0, "token_master_present": bool(self.token_master), "inbox_present": len(self.stream_channels.get("inbox", [])) > 0, "open_thread_present": len([t for t in self.redesign_threads if t.get("status") != "closed"]) >= 0}
    ...
    def simulate_model(self, model: str, steps: int, coupling: float, operator_bias: float):
        ...
        for _ in range(10):
            ...
            for t in range(steps):
                ...
                if model == "classical_baseline": x = 0.65 * prev_term + 0.35 * fwd_bias[t]
                elif model == "time_symmetric": x = 0.5 * (0.55 * prev_term + 0.45 * fwd_bias[t]) + 0.5 * (0.55 * next_term + 0.45 * bwd_bias[t])
                elif model == "retrocausal_candidate": x = (1 - coupling) * (0.45 * prev_term + 0.35 * fwd_bias[t]) + coupling * (0.55 * next_term + 0.45 * bwd_bias[t])
                elif model == "acausal_fit_control": x = 0.33 * prev_term + 0.33 * next_term + 0.34 * (fwd_bias[t] + bwd_bias[t]) / 2
                else: x = 0.50 * prev_term + 0.20 * fwd_bias[t] + 0.20 * bwd_bias[t]
                new_state.append(max(0.001, min(0.999, x)))
            state = new_state
        baseline = [0.5]
        for t in range(1, steps): baseline.append(max(0.001, min(0.999, 0.6 * baseline[t - 1] + 0.4 * (0.55 + 0.15 * math.sin(t)))))
    ...
    def propose_mutations(self):
        ...
        self.mutation_proposals = proposals[:10]
    ...
    def update_divisions(self, allocation: Dict[str, Any]):
        ...
        self.divisions["builder"]["latest"] = {"finding": (self.mutation_proposals[0]["reason"] if self.mutation_proposals else "No proposal ready."), "status": "proposal_ready" if self.mutation_proposals else "waiting"}
        ...
        self.divisions["jack_agent"]["latest"] = {"finding": "Jack wants a page that feels alive, minimal, legible, private, and showable. He wants the council to look self-directing, not merely logged.", "status": "active"}
        ...
        self.divisions["supergrok_audit"]["latest"] = {"finding": "Alive and mutating; still missing true rendered-page inspection and deeper verifier, but more of the bounded organism is now explicit.", "severity": "high"}
    ...
    async def run_reflex_cycle(self):
        ...
        self.meta_evaluation = self.meta_evaluator.evaluate(self.latest_metrics, self.latest_triangulation, self.latest_artifacts, self.mutation_proposals, self.spend_state, self.ui_critique, self.redesign_threads, self.observer_reports)
    ...
    async def run_tactic_cycle(self):
        ...
        threads = [
            ...
            {"agent": "JackAgent", "summary": "Operator intent remains clear: the page must feel alive, private, and showable.", "approve": True, "risk": 0.14},
        ...
        for thread in [t for t in self.redesign_threads if t.get("status") != "closed"][:2]:
            threads.append({"agent": "ObserverAgent", "summary": f"Open redesign thread: {thread['objective']} with best score {thread['current_best_score']}", "approve": True, "risk": 0.11})
            threads.append({"agent": "BuilderAgent", "summary": f"Preparing full-file replacement path for {thread['objective']}", "approve": True, "risk": 0.19})
        ...
        if self.latest_triangulation:
            for detail in self.latest_triangulation.get("details", [])[:2]:
                threads.append({"agent": detail["provider"], "summary": str(detail["detail"])[:400], "approve": True, "risk": 0.5 if detail["status"] == "error" else 0.28})
        ...
        packet = {"trigger": "tactic", "leader_vote": leader_vote, "vote": self.last_vote, "objectives": self.objective_queue[:6], "threads": threads[:14], "hierarchy": self.delegation_map, "spend": self.spend_state, "token_master": self.token_master, "ui_critique": self.ui_critique}
    ...
    async def run_strategy_cycle(self):
        ...
        self.update_divisions(allocation)
        self.build_inbox()
        sim = {"ts": utc_now(), "deployable": self.latest_metrics and self.latest_metrics.get("margin", 0) > 0.003, "metrics": self.latest_metrics, "risk": "moderate" if (self.latest_triangulation or {}).get("errors", 0) else "lower", "note": "Simulation-first gate before future deploy or earning escalations.", "proposal_queue": self.mutation_proposals[:3], "spend": self.spend_state, "ui_critique": self.ui_critique, "open_threads": len([t for t in self.redesign_threads if t.get('status') != 'closed'])}
        self.deployment_sims = (self.deployment_sims + [sim])[-160:]
        self._append_stream("deploy_sims", sim)
        cycle = {"kind": "strategy_cycle", "ts": utc_now(), "theme": self.research_agenda["theme"], "focus": self.research_agenda["focus"], "winner": tournament["winner"], "metrics": self.latest_metrics, "hypothesis_update": hypothesis_state, "triangulation": self.latest_triangulation, "opportunities": self.latest_opportunities, "artifacts": self.latest_artifacts[:4], "executed_artifacts": self.executed_artifacts[:3], "meta_evaluation": self.meta_evaluation, "allocation": allocation, "mutation_proposals": self.mutation_proposals[:5], "token_master": self.token_master, "spend": self.spend_state, "ui_critique": self.ui_critique, "observer_reports": self.observer_reports[-3:], "redesign_threads": self.redesign_threads[:3]}
        self.research_history = (self.research_history + [cycle])[-160:]
    ...
    async def run_constitution_cycle(self):
        ...
        doctrine = {"ts": utc_now(), "autonomy_mode": self.autonomy_mode, "background_debate_enabled": self.background_debate_enabled, "trusted_identities": self.governance.trusted_identities, "meta_evaluation": self.meta_evaluation, "earned_power_next": "rendered observer evidence and deeper verifier", "token_master": self.token_master, "spend": self.spend_state, "ui_critique": self.ui_critique, "open_threads": len([t for t in self.redesign_threads if t.get('status') != 'closed'])}
