    ...
    def build(self, open_questions: List[str], opportunities: List[Dict[str, Any]], recent_questions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx, (kind, text) in enumerate(sources[:8], start=1):
            goals.append({
                ...
                "kind": kind,
                "goal": text,
                "priority": round(max(0.45, 1.0 - 0.07 * (idx - 1)), 2),
...
class MetaEvaluator:
    def evaluate(self, metrics: Optional[Dict[str, Any]], triangulation: Optional[Dict[str, Any]], artifacts: List[Dict[str, Any]]) -> Dict[str, Any]:
        metrics = metrics or {}
        ...
        return {
            ...
            "answer": "A more visible, persistent, budget-aware institution with stronger replay and artifact production." if triangulation.get("successes", 0) >= 1 else "A reliability-first institution that heals its action surfaces before scaling.",
        }


class SnapshotReplay:
    def snapshot(self, label: str, autonomy_mode: str, background_debate_enabled: bool, latest_metrics: Optional[Dict[str, Any]], latest_triangulation: Optional[Dict[str, Any]], leader: str, objective_queue: List[Dict[str, Any]]) -> Dict[str, Any]:
        return {
            "ts": utc_now(),
            "label": label,
...
class AutonomousInstitutionEngine:
    ...
    def _append_meeting(self, kind: str, content: Dict[str, Any]):
        self.meeting_stream = (self.meeting_stream + [{"ts": utc_now(), "kind": kind, "content": content}])[-200:]

    def _append_question(self, division: str, question: str):
        self.self_questions = (self.self_questions + [{"ts": utc_now(), "division": division, "question": question}])[-400:]

    def snapshot(self, label: str):
        snap = self.snapshot_replay.snapshot(label, self.autonomy_mode, self.background_debate_enabled, self.latest_metrics, self.latest_triangulation, self.governance.leader, self.objective_queue)
    ...
    def update_divisions(self, allocation: Dict[str, Any]):
        ...
        self.divisions["expansion"]["latest"] = {"finding": "Mutation arm restored; next compounding target is richer browser replay and deploy sims.", "priority": 1}
        ...
        self.divisions["supergrok_audit"]["latest"] = {"finding": "Organism is alive but still needs more file-level decomposition and stronger replay persistence.", "severity": "high"}
    ...
    async def run_reflex_cycle(self):
        ...
        self.meta_evaluation = self.meta_evaluator.evaluate(self.latest_metrics, self.latest_triangulation, self.latest_artifacts)
    ...
    async def run_tactic_cycle(self):
        ...
        packet = {"trigger": "tactic", "leader_vote": leader_vote, "vote": self.last_vote, "objectives": self.objective_queue[:6], "threads": threads[:6]}
