...
class AutonomousInstitutionEngine:
    ...
    def _derive_objective(self) -> str:
        ...
        if not self.genesis_triggered:
            seeds = ["Achieve first successful autonomous mutation to trigger Genesis event"]
            seeds += open_thread_labels[:2]
        else:
    ...
    async def _enforce_probation(self, p: Dict, resumed: bool = False):
        ...
        if verified:
            ...
            self.cog.record_outcome(objective, touched_modules, False,
                                    rollback_reason=detail.get("note") or str(detail.get("reason", "probation_timeout")))
    ...
    async def _loop_mutation(self):
        if os.getenv("IS_SHADOW") == "true" or os.getenv("AUTONOMY_ENABLED") != "true":
            return
        await asyncio.sleep(90)
        while True:
            try:
                if self.mutation_status == "IDLE" and self.background_debate_enabled:
                    if self.autonomy_mode in ("autonomous", "free"):
    ...
    async def _loop_free_agency(self):
        """When free agency is enabled, agents autonomously generate their own directives."""
