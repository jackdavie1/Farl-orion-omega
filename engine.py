...
class LedgerClient:
    def __init__(self, base_url: str):
        # Strip /log suffix if present — LEDGER_URL may be set to .../log but
        # the ledger service has routes at /log, /latest, /entries (not /log/log)
...
class AutonomousInstitutionEngine:
    ...
    async def start(self):
        ...
        boot_msg = (f"Orion Apex v18 online | deployer={'ready' if self.deployer else 'MISSING'} | "
                    f"autonomy={os.getenv('AUTONOMY_ENABLED', 'false')} | "
                    f"grok={'ready' if os.getenv('XAI_API_KEY') else 'MISSING'}")
        self._meet("governance", {"event": "boot", "message": boot_msg})
