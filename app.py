...
def _live() -> Dict:
    ...
    return {
        "summary": {
            ...
            "deployer_diagnostics": s.get("deployer_diagnostics", {}),
            "debate_cycles": engine.world_model.get("metrics", {}).get("debate_cycles", 0) if hasattr(engine, 'world_model') else 0,
        },
        "queues": {
...
async def api_code_gen(request: Request):
    """Generate production-ready code. The sellable product."""
    try:
        body = await request.json()
        task = body.get("task", "")
        language = body.get("language", "python")
        if not task:
            return JSONResponse({"error": "task required"}, status_code=400, headers=nc())
        result = await engine.generator._smart(
            f"You are an expert {language} developer. Write clean, production-ready code. "
            "Return ONLY the code with a brief docstring. No markdown fences.",
            f"Task: {task}\nLanguage: {language}\nWrite the complete implementation:",
            max_tokens=2000, context="api_code_gen"
        )
        if result.startswith('{"error"'):
            return JSONResponse({"error": "LLM unavailable"}, status_code=503, headers=nc())
        return JSONResponse({"code": result, "language": language, "task": task[:100],
                             "agent": "BuilderAgent"}, headers=nc())
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500, headers=nc())


@app.post("/api/research")
async def api_research(request: Request):
    """Multi-agent research report. 5 agents debate any topic."""
    try:
        body = await request.json()
        topic = body.get("topic", "")
        if not topic:
            return JSONResponse({"error": "topic required"}, status_code=400, headers=nc())
        research = await engine.generator.run_research_cycle(
            {"last_mutation_objective": topic, "fragility": 0, "failure_streak": 0}
        )
        responses = await engine.generator.council_respond(
            f"Provide a research report on: {topic}. "
            "Give analysis, key insights, and actionable conclusions.",
            engine.get_state()
        )
        report = (
            f"# Research Report: {topic}\n\n"
            f"## Framework\n{research.get('direction', '')}\n\n"
            f"## Key Insight\n{research.get('insight', '')}\n\n"
            f"## Agent Analysis\n" +
            "\n\n".join(f"**{r['agent']}**: {r['message']}" for r in responses)
        )
        return JSONResponse({"topic": topic, "report": report,
                             "agents": [r["agent"] for r in responses]}, headers=nc())
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500, headers=nc())


@app.post("/api/debate")
async def api_debate(request: Request):
    """Council debate on any question. Returns structured agent responses."""
    try:
        body = await request.json()
        question = body.get("question", "")
        if not question:
            return JSONResponse({"error": "question required"}, status_code=400, headers=nc())
        responses = await engine.generator.council_respond(question, engine.get_state())
        return JSONResponse({"question": question, "responses": responses,
                             "agent_count": len(responses)}, headers=nc())
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500, headers=nc())
...
async def api_status():
    """Public — shows FARL is live. Link this from Gumroad."""
    return JSONResponse({
        "status": "LIVE", "system": "FARL Orion Apex",
        "endpoints": ["/api/code-gen", "/api/research", "/api/debate"],
        "agents": ["Signal","Vector","BuilderAgent","Supergrok","Guardian","TokenMaster"],
        "mutation_status": engine.mutation_status,
    }, headers=nc())
...
<div id="shell">
  ...
  <div id="pills">
    ...
    <div class="pill" id="pEarn">💰0</div>
    <div class="pill" id="pSR">✓0%</div>
  </div>
...
function fmtGov(c){
  const e=c.event||"";
  if(e==="boot")return`🚀 ${c.message||"System online"}`;
  ...
  if(e==="probation_started")return`🔬 Probation: ${(c.objective||"?").slice(0,50)} SHA:${(c.sha||"?").slice(0,8)}`;
  ...
  if(e==="meta_mode_changed")return`⚙ Meta → ${c.mode||"?"}: ${c.mode_reason||""}`;
  if(e==="shadow_result")return`${c.ok?"✓":"✗"} Shadow: ${c.reason||""}`;
...
function renderRail(){
  ...
  rb.innerHTML=`
    ...
    <div class="sc">
      ...
      ${!dep?`<div class="re" style="color:var(--bad)">GH:${diag.github_token_set?"✓":"✗"} REPO:${diag.repo_name_set?"✓":"✗"} GROK:${diag.xai_key_set?"✓":"✗"}</div>`:""}
      </div>
    </div>
    ...
    <div class="sc">
      <div class="pt">Metrics</div>
      <div class="ri"><div class="rt">Debate cycles: ${s.debate_cycles||0}</div></div>
      <div class="ri"><div class="rt">Earning opps: ${s.earning_opps_found||0}</div></div>
      <div class="ri"><div class="rt">Bridge pending: ${s.bridge_pending||0}</div></div>
    </div>
  `;
}
