...
def compact(obj: Any, limit: int = 1600) -> str:
    return json.dumps(obj, separators=(",", ":"), default=str)[:limit]
...
def inspect_rendered_view_html(html: str) -> Dict[str, Any]:
    checks = {
        "has_rooms": "roomList" in html,
        "has_chat": "id='chat'" in html or 'id="chat"' in html,
        "has_inbox": "dmList" in html,
    ...
    status = "healthy" if score >= 0.875 else "degraded" if score >= 0.5 else "critical"
    return {"score": score, "status": status, "checks": checks, "summary": "Rendered page appears structurally sound." if status == "healthy" else "Rendered page still appears structurally weak."}
...
def synthesize_view_dashboard_function() -> str:
    ...
    m = re.search(pattern, current, re.DOTALL)
    if not m:
        raise ValueError("current_view_dashboard_not_found")
    snippet = m.group(0)
    return snippet[: -len('\n\n@app.get("/view/state")')]
...
def replace_view_function(app_text: str, replacement_func: str) -> str:
    ...
    new_text, count = re.subn(pattern, replacement, app_text, flags=re.DOTALL)
    if count != 1:
...
async def run_view_rebuild_closure(source: str, authorized_by: str) -> Dict[str, Any]:
    ...
    rebuilt_app = replace_view_function(current_app, synthesize_view_dashboard_function())
    ...
    if post_inspect.get("score", 0.0) < pre_inspect.get("score", 0.0) or post_inspect.get("status") == "critical" or engine.rollback_recommended(verify):
        ...
        engine.record_regression("view_rebuild", "Rendered verification degraded after /view rebuild.", pre_inspect, post_inspect)
    else:
...
async def run_self_tuning_closure(source: str, authorized_by: str) -> Dict[str, Any]:
    global LAST_AUTO_PUSH_TS
    state = engine.get_state()
    gate = should_attempt_autonomous_push(state)
    engine._append_stream("governance", {"autonomy_gate": gate, "ts": utc_now()})
    ...
    if engine.rollback_recommended(verify):
        ...
        engine.record_regression("self_tuning", "Self-tuning closure rolled back after verification.", simulation, verify)
    else:
...
async def run_autonomous_implementation(source: str, authorized_by: str) -> Dict[str, Any]:
    ...
    if any("/view" in (t.get("objective", "") or "") or "control room" in (t.get("objective", "") or "").lower() for t in open_threads):
        return await run_view_rebuild_closure(source, authorized_by)
    return await run_self_tuning_closure(source, authorized_by)


async def autonomous_operator_loop() -> None:
    while True:
        try:
...
async def view_stream():
    return JSONResponse(
        {"channels": engine.stream_channels, "meetings": engine.meeting_stream[-80:], "questions": engine.self_questions[-80:], "snapshots": engine.snapshots[-40:], "deployment_sims": engine.deployment_sims[-40:]},
...
async def agent_propose(body: BusRequest):
    ...
    try:
        ...
        if command == "OPERATOR_CLEAR_CHAT":
            ...
            for key in list(engine.stream_channels.keys()):
                engine.stream_channels[key] = []
        ...
        if command == "LEDGER_WRITE":
            ...
            return envelope(result["ok"], result["data"], None if result["ok"] else f"Ledger write failed: {result.get('status_code')}")
        ...
        if command == "SET_CONSTRAINTS":
            ...
            await engine.write_ledger(
                "COUNCIL_SYNTHESIS",
                {
                    ...
                    "snapshot": compact(snap),
                },
            )
            if body.enabled and (body.mode or engine.autonomy_mode) == "autonomous":
