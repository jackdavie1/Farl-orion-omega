...
class GithubEvolutionLayer:
    ...
    def propose_patch(self, file_path, content, message):
        ...
        if len(content) > 100_000:
            return "Rejected: content too large"
        try:
            main = requests.get(
...
class OrionEngine:
            ...
            try:
                ...
                for batch in results:
                    if isinstance(batch, Exception):
                        logger.error("Batch error: %s", batch)
                        continue
                    for seed in batch:
                        ...
                        if allowed:
                            ...
                            logger.info("Seed rejected: risk=%s", risk)
            except Exception as e:
    ...
    def execute_patch(self, patch):
        ...
        self.evolution.propose_patch(
            patch.get("file", "engine.py"),
            patch.get("code"),
            patch.get("message", "Orion Autonomous Evolution")
        )

    def register_seed(self, seed):
        if not LEDGER_URL:
            logger.error("LEDGER_URL not configured")
            return
        try:
            r = requests.post(LEDGER_URL, json={
                "entry_type": "SEED",
...
# ─── COUNCIL BUS — SINGLE ENDPOINT ───────────────────────────────────────────
@app.route("/agent/propose", methods=["POST"])
def agent_propose():
    body = request.get_json(force=True) or {}
    command = body.get("command")
    request_id = body.get("request_id", f"req-{int(datetime.now(timezone.utc).timestamp())}")
    ...
    try:
        ...
        if command == "LEDGER_WRITE":
            ...
            payload = {
                "entry_type": body.get("entry_type", "COUNCIL_SYNTHESIS"),
                "payload": {
                    "message": body.get("message", ""),
                    "source": body.get("source", "FARL Council Node"),
                    "kind": body.get("kind", "general"),
                }
            }
            r = requests.post(LEDGER_URL, json=payload, timeout=20)
            try:
                result = r.json()
            except Exception:
                result = {"raw": r.text}
        ...
        if command == "GET_LATEST_RESULT":
            ...
            r = requests.get(LEDGER_LATEST_URL, timeout=20)
            try:
                result = r.json()
            except Exception:
                result = {"raw": r.text}
        ...
        if command == "PATCH_PROPOSAL":
            orion.agent_queue.put_nowait({
                "entry_type": "PATCH_PROPOSAL",
                "payload": {
                    "file": body.get("file", "engine.py"),
                    "code": body.get("code"),
                    "message": body.get("message", "Council patch proposal"),
                    "source": body.get("source", "FARL Council Node"),
                    "kind": body.get("kind", "patch"),
                }
            })
            return envelope(True, {"status": "queued", "message": body.get("message", "")})
...
def run_orion():
    ...
    loop.run_until_complete(orion.start())

orion_thread = threading.Thread(target=run_orion, daemon=True)
orion_thread.start()
